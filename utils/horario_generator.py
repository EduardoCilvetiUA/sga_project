...
class HorarioGenerator:
    def __init__(self):
        ...
        self.horas_inicio = ['09:00', '10:00', '11:00', '12:00', '14:00', '15:00', '16:00', '17:00']
    ...
    def check_profesor_disponible(self, profesor_id, dia, hora_inicio, hora_fin):
        ...
        conflictos = execute_query(
            check_professor_availability, 
            (profesor_id, dia, hora_inicio, hora_inicio, hora_fin, hora_fin, hora_inicio, hora_fin),
            fetch=True
        )
        return conflictos[0]['count'] == 0
    ...
    def check_alumno_disponible(self, alumno_id, dia, hora_inicio, hora_fin):
        ...
        conflictos = execute_query(
            check_student_availability, 
            (alumno_id, dia, hora_inicio, hora_inicio, hora_fin, hora_fin, hora_inicio, hora_fin),
            fetch=True
        )
        return conflictos[0]['count'] == 0
    ...
    def asignar_horario(self, seccion_id, sala_id, dia, hora_inicio, hora_fin):
        ...
        execute_query(assign_schedule, (seccion_id, sala_id, dia, hora_inicio, hora_fin))
        ...
        for seccion in secciones:
            ...
            for dia in dias_random:
                ...
                for hora_inicio_str in horas_random:
                    ...
                    if hora_inicio < datetime.strptime('13:00', '%H:%M').time() and hora_fin > datetime.strptime('13:00', '%H:%M').time():
                        continue
                    ...
                    for sala in salas:
                        ...
                        if not self.check_sala_disponible(sala_id, dia, hora_inicio, hora_fin):
                            continue
                            
                        # Verificar disponibilidad de profesores
                        profesores_disponibles = True
                        for profesor in profesores:
                            if not self.check_profesor_disponible(profesor['id'], dia, hora_inicio, hora_fin):
                        ...
                        for alumno in alumnos:
                            if not self.check_alumno_disponible(alumno['id'], dia, hora_inicio, hora_fin):
                        ...
                        self.asignar_horario(seccion_id, sala_id, dia, hora_inicio, hora_fin)
    ...
    def exportar_horarios_excel(self, anio, periodo, file_path):
        ...
        try:
            ...
            horarios = execute_query(get_schedules_for_export, (anio, periodo), fetch=True)
            ...
            with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
                ...
                df_salas.sort_values(['sala_nombre', 'dia', 'hora_inicio'], inplace=True)
                df_salas.to_excel(writer, sheet_name='Salas', index=False)
                
                # Hoja 3: Horarios por curso
                df_cursos = df.copy()
                df_cursos.sort_values(['curso_codigo', 'seccion_numero', 'dia', 'hora_inicio'], inplace=True)
                df_cursos.to_excel(writer, sheet_name='Cursos', index=False)
                
                # Hoja 4: Horarios por profesor
                df_profesores = df.copy()
                df_profesores.sort_values(['profesor_nombre', 'dia', 'hora_inicio'], inplace=True)
