...
class JsonLoader:
    ...
    def _process_entity(
        ...
        required_fields,
        format_insert_data,
        format_update_data=None,
    ):
        if entity_key not in data:
            raise ValueError(
                f"El archivo JSON no contiene la estructura esperada para {entity_key}"
            )
    ...
    def load_cursos(file_path):
        ...
        for curso in data["cursos"]:
            try:
                entity_id = JsonLoader._check_required_fields(
                    curso, ["id", "codigo", "descripcion"], "curso"
                )
    ...
    def load_instancias_cursos(file_path):
        ...
        for instancia in data["instancias"]:
            try:
                entity_id = JsonLoader._check_required_fields(
                    instancia, ["id", "curso_id"], "instancia"
                )
                ...
                existing = execute_query(
                    check_instancia_exists, (entity_id,), fetch=True
                )
    ...
    def load_secciones(file_path):
        ...
        for seccion in data["secciones"]:
            try:
                entity_id = JsonLoader._check_required_fields(
                    seccion,
                    ["id", "instancia_curso", "profesor_id", "evaluacion"],
                    "sección",
                )
    ...
    def load_notas(file_path):
        ...
        for nota_data in data["notas"]:
            try:
                ...
                if not execute_query(
                    check_topico_exists_by_id, (topico_id,), fetch=True
                ):
