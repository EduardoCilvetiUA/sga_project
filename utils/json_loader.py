...
class JsonLoader:
    ...
    def load_alumnos(file_path):
        ...
        if 'alumnos' not in data:
            raise ValueError("El archivo JSON no contiene la estructura esperada para alumnos")
        ...
        for alumno in data['alumnos']:
            try:
                ...
                if not all(k in alumno for k in ['id', 'nombre', 'correo', 'anio_ingreso']):
                    raise ValueError(f"Alumno con ID {alumno.get('id', 'desconocido')} no tiene todos los campos requeridos")
                
                # Verificar si el alumno ya existe
                existing = execute_query(check_alumno_exists, (alumno['id'],), fetch=True)
                
                if existing:
                    # Actualizar alumno existente
                    execute_query(update_alumno, 
                        (alumno['nombre'], alumno['correo'], f"{alumno['anio_ingreso']}-01-01", alumno['id'])
                    )
                else:
                    # Insertar nuevo alumno con ID específico
                    execute_query(insert_alumno, 
                        (alumno['id'], alumno['nombre'], alumno['correo'], f"{alumno['anio_ingreso']}-01-01")
                    )
                ...
                result['errores'].append({
                    'alumno_id': alumno.get('id', 'desconocido'),
                    'error': str(e)
                })
    ...
    def load_profesores(file_path):
        ...
        if 'profesores' not in data:
            raise ValueError("El archivo JSON no contiene la estructura esperada para profesores")
        ...
        for profesor in data['profesores']:
            try:
                ...
                if not all(k in profesor for k in ['id', 'nombre', 'correo']):
                    raise ValueError(f"Profesor con ID {profesor.get('id', 'desconocido')} no tiene todos los campos requeridos")
                
                # Verificar si el profesor ya existe
                existing = execute_query(check_profesor_exists, (profesor['id'],), fetch=True)
                
                if existing:
                    # Actualizar profesor existente
                    execute_query(update_profesor, 
                        (profesor['nombre'], profesor['correo'], profesor['id'])
                    )
                else:
                    # Insertar nuevo profesor con ID específico
                    execute_query(insert_profesor, 
                ...
                result['fallidos'] += 1
                result['errores'].append({
                    'profesor_id': profesor.get('id', 'desconocido'),
                    'error': str(e)
                })
        ...
        for curso in data['cursos']:
            try:
                ...
                if not all(k in curso for k in ['id', 'codigo', 'descripcion']):
                    raise ValueError(f"Curso con ID {curso.get('id', 'desconocido')} no tiene todos los campos requeridos")
                ...
                if existing:
                    ...
                    execute_query(update_curso, 
                        (curso['codigo'], curso['descripcion'], creditos, curso['id'])
                    )
                else:
                    # Insertar nuevo curso con ID específico
                    execute_query(insert_curso, 
                        (curso['id'], curso['codigo'], curso['descripcion'], creditos, False)
                    )
    ...
    def load_salas(file_path):
        ...
        if 'salas' not in data:
            raise ValueError("El archivo JSON no contiene la estructura esperada para salas")
        ...
        for sala in data['salas']:
            try:
                ...
                if not all(k in sala for k in ['id', 'nombre', 'capacidad']):
                    raise ValueError(f"Sala con ID {sala.get('id', 'desconocido')} no tiene todos los campos requeridos")
                ...
                if existing:
                    ...
                    execute_query(update_sala, 
                        (sala['nombre'], sala['capacidad'], sala['id'])
                    )
                else:
                    # Insertar nueva sala con ID específico
                    execute_query(insert_sala, 
                        (sala['id'], sala['nombre'], sala['capacidad'])
                    )
        ...
        for instancia in data['instancias']:
            try:
                ...
                if not all(k in instancia for k in ['id', 'curso_id']):
                    raise ValueError(f"Instancia con ID {instancia.get('id', 'desconocido')} no tiene todos los campos requeridos")
                
                # Verificar que el curso exista
                curso_exists = execute_query(check_curso_exists_by_id, 
                    (instancia['curso_id'],), 
                    fetch=True
                )
                ...
                existing = execute_query(check_instancia_exists, 
                    (instancia['id'],), 
                    fetch=True
                )
    ...
    def load_secciones(file_path):
        ...
        for seccion in data['secciones']:
            try:
                # Validar campos requeridos
                if not all(k in seccion for k in ['id', 'instancia_curso', 'profesor_id', 'evaluacion']):
                    raise ValueError(f"Sección con ID {seccion.get('id', 'desconocido')} no tiene todos los campos requeridos")
                
                # Verificar que la instancia de curso exista
                instancia_exists = execute_query(check_instancia_exists, 
                ...
                profesor_exists = execute_query(check_profesor_exists, 
                    (seccion['profesor_id'],), 
                    fetch=True
                )
                ...
                existing = execute_query(check_seccion_exists, 
                    (seccion['id'],), 
                    fetch=True
                )
                ...
                if existing:
                    ...
                    execute_query(update_seccion, 
                        (seccion['instancia_curso'], siguiente_numero, usa_porcentaje, seccion['id'])
                    )
                else:
                    # Insertar nueva sección con ID específico
                    execute_query(insert_seccion, 
                        (seccion['id'], seccion['instancia_curso'], siguiente_numero, usa_porcentaje)
                    )
                
                # Asignar profesor a la sección
                profesor_seccion_exists = execute_query(check_profesor_seccion_exists, 
                    (seccion['profesor_id'], seccion['id']), 
                    fetch=True
                )
                
                if not profesor_seccion_exists:
                    execute_query(insert_profesor_seccion, 
                        (seccion['profesor_id'], seccion['id'])
                    )
                ...
                if 'combinacion_topicos' in seccion['evaluacion']:
                    ...
                    if usa_porcentaje:
                        ...
                        if abs(total - 100) > 0.1:  # Permitir pequeño margen de error
                            ...
                            for i in range(len(valores)):
                                topicos[i]['valor'] = round(topicos[i]['valor'] * factor, 2)
                    ...
                    for topico in topicos:
                        ...
                        existing_topico = execute_query(check_topico_exists, 
                            (topico_id,), 
                            fetch=True
                        )
                        ...
                        if topico_usa_porcentaje:
                            ...
                            if abs(total_instancias - 100) > 0.1:  # Permitir pequeño margen de error
                                # Normalizar los porcentajes
                                factor = 100 / total_instancias
                                for i in range(len(valores_instancias)):
                                    valores_instancias[i] = round(valores_instancias[i] * factor, 2)
                        ...
                        for i in range(cantidad):
                            ...
                            execute_query(insert_instancia_evaluacion, 
                ...
                result['fallidos'] += 1
                result['errores'].append({
                    'seccion_id': seccion.get('id', 'desconocido'),
                    'error': str(e)
                })
    ...
    def load_alumnos_seccion(file_path):
        ...
        for asignacion in data['alumnos_seccion']:
            try:
                ...
                seccion_exists = execute_query(check_seccion_exists_by_id, 
                    (asignacion['seccion_id'],), 
                    fetch=True
                )
                
                if not seccion_exists:
                    raise ValueError(f"No existe una sección con ID {asignacion['seccion_id']}")
                    
                # Verificar que el alumno exista
                alumno_exists = execute_query(check_alumno_exists_by_id, 
                    (asignacion['alumno_id'],), 
                    fetch=True
                )
                
                if not alumno_exists:
                    raise ValueError(f"No existe un alumno con ID {asignacion['alumno_id']}")
                
                # Verificar si ya existe la asignación
                existing = execute_query(check_alumno_seccion_exists, 
                ...
                if not existing:
                    ...
                    execute_query(insert_alumno_seccion, 
                        (asignacion['alumno_id'], asignacion['seccion_id'])
                    )
    ...
    def load_notas(file_path):
        ...
        for nota_data in data['notas']:
            try:
                ...
                if not alumno_exists:
                    raise ValueError(f"No existe un alumno con ID {nota_data['alumno_id']}")
                
                # Verificar que el tópico exista
                topico_exists = execute_query(check_topico_exists_by_id, 
                    (nota_data['topico_id'],), 
                    fetch=True
                )
                
                if not topico_exists:
                    raise ValueError(f"No existe un tópico con ID {nota_data['topico_id']}")
                
                seccion_id = execute_query(get_seccion_id_from_topico, 
                    (nota_data['topico_id'],), 
                    fetch=True
                )
                
                if not seccion_id:
                    raise ValueError(f"No se pudo obtener la sección del tópico {nota_data['topico_id']}")
                
                seccion_id = seccion_id[0]['seccion_id']
                
                # Verificar que el alumno esté inscrito en la sección
                alumno_seccion = execute_query(check_alumno_seccion_by_seccion, 
                    (nota_data['alumno_id'], seccion_id), 
                    fetch=True
                )
                
                if not alumno_seccion:
                    raise ValueError(f"El alumno con ID {nota_data['alumno_id']} no está inscrito en la sección del tópico {nota_data['topico_id']}")
                
                alumno_seccion_id = alumno_seccion[0]['id']
                
                # Obtener la instancia de evaluación
                instancia_evaluacion = execute_query(get_instancia_evaluacion_by_offset, 
                    (nota_data['topico_id'], nota_data['instancia'] - 1), 
                    fetch=True
                )
                
                if not instancia_evaluacion:
                    raise ValueError(f"No existe la instancia {nota_data['instancia']} para el tópico {nota_data['topico_id']}")
                ...
                existing = execute_query(check_nota_exists, 
                    (alumno_seccion_id, instancia_id), 
                    fetch=True
                )
                
                if existing:
                    # Actualizar nota existente
                    execute_query(update_nota, 
                        (nota_valor, existing[0]['id'])
                    )
                else:
                    # Insertar nueva nota
                    execute_query(insert_nota, 
                        (alumno_seccion_id, instancia_id, nota_valor)
                    )
